...
class NullUniversalModule(nn.Module):
    ...
    def _forward(self, x):
        ...
        # Fix for FP8 Base: Avoid FP8 arithmetic in LoRA path
        if hasattr(torch, "float8_e4m3fn") and dtype in [torch.float8_e4m3fn, torch.float8_e5m2]:
        ...
        gate_scores = None
        if self.num_experts > 1:
            # Fix for Float8: ensure mean is calculated in float32
            x_calc = x.to(torch.float32)
            if self.is_conv:
                gate_input = x_calc.mean(dim=(2, 3))
            else:
                gate_input = x_calc.mean(dim=1) if x_calc.dim() == 3 else x_calc
            
            self.gate.to(device)
            gate_logits = self.gate(gate_input.to(dtype))
            if self.training:
                # Fix for Float8: Perform softmax and masking in float32
                mask = self.expert_mask.to(device=device, dtype=torch.float32).view(1, -1)
                gate_logits_f32 = gate_logits.to(torch.float32)
                gate_scores = F.softmax(gate_logits_f32, dim=-1) * mask
                gate_scores = gate_scores / (gate_scores.sum(dim=-1, keepdim=True) + 1e-6)
                gate_scores = gate_scores.to(dtype)
            else:
                gate_scores = F.softmax(gate_logits.to(torch.float32), dim=-1).to(dtype)
        
        # Prepare Scaling
        s1, s2 = 1.0, 1.0
        if not self.use_dora:
            # Fix for Float8: mean() is not implemented for Float8
            s_src = self.s.to(device=device, dtype=torch.float32)
            s1 = s_src[:self.r_half].mean().to(dtype) # Simplify to scalar for mixing
            s2 = s_src[self.r_half:].mean().to(dtype)
        ...
        if self.abm_enabled and self.training:
             ...
             with torch.no_grad():
                 ...
                 # Fix for Float8: calculate loss in float32
                 loss_val = torch.mean(hinge.to(torch.float32).pow(2)) * self.abm_weight
...
class VDropNullUniversalNetwork(nn.Module):
    def __init__(self, text_encoder, unet, multiplier=1.0, lalora_lambda=0.0, use_dora=False, enable_effilora=False, num_experts=4, loraplus_lr_ratio=16.0, abm_batch_size=None, dino_dim=1024, text_dim=2048):
        super().__init__()
        self.multiplier = multiplier
        self.lalora_lambda = lalora_lambda
        self.use_dora = use_dora
    ...
    def load_basis_and_init(self, basis_path, lalora_context_path=None, weights_path=None, svd_cache_path=None, total_rank=32, ignore_te=False, storage_dtype=torch.float32):
        ...
        if loaded_weights:
            ...
            if proj_weights:
                self.projector.load_state_dict(proj_weights)
    ...
    def prepare_optimizer_params(self, text_encoder_lr, unet_lr, default_lr, loraplus_lr_ratio=None):
        ...
        # Add projector params (train with unet lr)
        self.projector.requires_grad_(True)
        all_params.append({"params": self.projector.parameters(), "lr": unet_lr_base})

        return all_params, ["alpha_down", "alpha_up", "s", "m", "gate", "projector"]
    ...
    def save_weights(self, file, dtype, metadata):
        ...
        for name, param in self.projector.named_parameters():
             state_dict[f"projector.{name}"] = to_cpu(param)
    ...
    def init_slao(self, slao_path):
        if not slao_path or not os.path.exists(slao_path):
            logger.warning(f"[SLAO] Path not found or empty: {slao_path}. Skipping SLAO init (treating as Task 1).")
            return

        logger.info(f"[SLAO] Initializing from previous task: {slao_path}")
        if slao_path.endswith(".safetensors"):
            loaded_weights = load_file(slao_path)
        else:
            loaded_weights = torch.load(slao_path, map_location="cpu")
        ...
        for name, module in self.modules_dict.items():
            ...
            ad_prev = find_key("alpha_down")
            au_prev = find_key("alpha_up")
            gate_w_prev = find_key("gate.weight")
            gate_b_prev = find_key("gate.bias")
            m_prev = find_key("m")
            s_prev = find_key("s")

            # SLAO Step 1: Initialize B (Up) from B_prev
            if au_prev is not None:
                if au_prev.shape == module.alpha_up.shape:
                    module.alpha_up.data.copy_(au_prev)
                elif au_prev.dim() == 2 and module.alpha_up.dim() == 3:
                     # If previous was single expert and now multiexpert, init all experts same? 
                     # Or just first? SLAO assumes single LoRA usually.
                     # Let's copy to all experts to be safe/stable
                    module.alpha_up.data[:] = au_prev.unsqueeze(0)

            # SLAO Step 2: Initialize A (Down) via Orthogonal Basis of A_prev
            # QR(A^T) -> Q. A_new = Q^T
            if ad_prev is not None:
                # ad_prev is (r, r) usually in this module
                # Orthogonalize rows of ad_prev
                # Q, R = torch.linalg.qr(ad_prev.T)
                # ad_new = Q.T
                
                try:
                    # We want A_new * A_prev^T = 0? No, that's for O-LoRA.
                    # SLAO: Q_i R_i = QR(A_{ft,i-1}^T). A_{ft,i}^{(0)} = Q_i^T.
                    # ensuring A_new * A_new^T = I.
                    
                    dtype = ad_prev.dtype
                    device = module.alpha_down.device
                    
                    # Compute QR on Transpose
                    # ad_prev: (Rank, Rank)
                    qt_local, _ = torch.linalg.qr(ad_prev.to(torch.float32).t())
                    ad_new = qt_local.t().to(dtype=dtype, device=device)
                    
                    module.alpha_down.data.copy_(ad_new)
                    
                except Exception as e:
                    logger.warning(f"[SLAO] QR Decomposition failed for {name}: {e}")

            # Copy other params to maintain state
            if module.gate is not None and gate_w_prev is not None:
                module.gate.weight.data.copy_(gate_w_prev)
                if gate_b_prev is not None: module.gate.bias.data.copy_(gate_b_prev)

            if self.use_dora:
                if m_prev is not None and hasattr(module, "m"): module.m.data.copy_(m_prev)
            else:
                if s_prev is not None and hasattr(module, "s"): module.s.data.copy_(s_prev)

            count += 1
        
        logger.info(f"[SLAO] Applied Orthogonal Initialization to {count} modules.")
...
def create_network(multiplier, network_dim, network_alpha, vae, text_encoder, unet, **kwargs):
    ...
    dino_dim = int(kwargs.get("dino_dim", 1024))
    text_dim = int(kwargs.get("text_dim", 2048))
    
    # SLAO Arguments
    enable_slao = str(kwargs.get("enable_slao", False)).lower() == "true"
    slao_path = kwargs.get("slao_path", None)

    if basis_path is None: raise ValueError("Null-Universal Network requires 'basis_path'")
    if network_dim is None: network_dim = 32 
    
    network = VDropNullUniversalNetwork(
        text_encoder, unet, 
    ...
    # Apply SLAO Initialization if enabled and path provided
    if enable_slao:
        network.init_slao(slao_path)

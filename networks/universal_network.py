...
class UniversalModule(nn.Module):
    def __init__(self, lora_name, org_module, mean_down, basis_down, mean_up, basis_up, multiplier=1.0, rank=16):
        super().__init__()
        self.lora_name = lora_name
        self.multiplier = multiplier
        self.org_module = org_module
        
        # Buffers: Fixed basis (Not trainable)
        ...
        # Parameter: Learnable coefficients (alpha)
...
class UniversalNetwork(nn.Module):
    def __init__(self, text_encoder, unet, multiplier=1.0):
        super().__init__()
        self.multiplier = multiplier
        self.modules_dict = nn.ModuleDict()
        
        # Handle list of text encoders (SDXL)
    ...
    def load_basis(self, basis_path):
        ...
        logger.info(f"Universal Network: Initialized {count} modules.")
    ...
    def enable_gradient_checkpointing(self):
        # Required by sd-scripts trainer
        pass
        
    def get_trainable_params(self):
        # Required by sd-scripts trainer for gradient clipping etc.
...
def create_network(multiplier, network_dim, network_alpha, vae, text_encoder, unet, **kwargs):
    network = UniversalNetwork(text_encoder, unet, multiplier=multiplier)
    if "basis_path" not in kwargs:
        raise ValueError("Universal Network requires 'basis_path' in --network_args")
    network.load_basis(kwargs["basis_path"])
    return network
